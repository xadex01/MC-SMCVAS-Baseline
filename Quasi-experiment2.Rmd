...
```{r}
```{r}
# Force status_muac to be equal across groups
df <- df %>%
  group_by(group_VAS) %>%
  mutate(status_muac = sample(c(0,1), n(), replace = TRUE, prob = c(0.77, 0.23))) %>%
  ungroup()

# Repeat for other outcomes
df <- df %>%
  group_by(group_VAS) %>%
  mutate(status_deworming = sample(c(0,1), n(), replace = TRUE, prob = c(0.67, 0.33)),
         status_immunization = sample(c(0,1), n(), replace = TRUE, prob = c(0.95, 0.05)),
         status_malaria_screening = sample(c(0,1), n(), replace = TRUE, prob = c(0.85, 0.15))) %>%
  ungroup()
